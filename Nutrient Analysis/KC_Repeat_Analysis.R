...
N_avg_diff <- sapply(N_1[Year > 2017], function(x) median(x, na.rm = TRUE)) - sapply(N_1[Year <= 2017], function(x) median(x, na.rm = TRUE))
N_avg_slp1 <- as.data.frame(N_avg_diff[-1]*10/N_avg_diffyr) #This is the average slope. Units are μmicrogram/Liter/year (μg/L/decade)
colnames(N_avg_slp1) <- c('Avg Slope (μg/L/decade)')

# Slope distribution Curve 
# ggplot(N_avg_slp1, aes(x = `Avg Slope (μg/L/decade)`)) +
...
P_avg_diff <- sapply(P_1[Year > 2017], function(x) median(x, na.rm = TRUE)) - sapply(P_1[Year <= 2017], function(x) median(x, na.rm = TRUE))
P_avg_slp1 <- as.data.frame(P_avg_diff[-1]*10/P_avg_diffyr) #This is the average slope. Units are microgram/Liter/year
colnames(P_avg_slp1) <- c('Avg Slope (μg/L/decade)')


# Slope distribution Curve 
#ggplot(P_avg_slp1, aes(x = `Avg Slope (μg/L/decade)`)) +
#  geom_histogram(stat = 'density') +
...
median_slopes[1,] <- c(median(N_avg_slp1[,1]), median(P_avg_slp1[,1]))
slopeSD[1,] <- c(sd(N_avg_slp1[,1]),sd(P_avg_slp1[,1]))
mean_slopes[1,] <- c(mean(N_avg_slp1[,1]), mean(P_avg_slp1[,1]))
...
# Calculates the baseline and recent averages, then calculates the difference
N_avg_diff <- sapply(N_2[Year > 2017], function(x) median(x, na.rm = TRUE)) - sapply(N_2[Year <= 2017], function(x) median(x, na.rm = TRUE))
N_avg_slp2 <- as.data.frame(N_avg_diff[-1]*10/N_avg_diffyr) #This is the average slope. Units are μmicrogram/Liter/year (μg/L/decade)
colnames(N_avg_slp2) <- c('Avg Slope (μg/L/decade)')

# Slope distribution histogram
ggplot(N_avg_slp2, aes(x = `Avg Slope (μg/L/decade)`)) +
  geom_histogram(binwidth = 35) + 
  geom_vline(xintercept = 0, linetype = 'twodash', color = 'grey', size = 1) +
...
P_avg_diff <- sapply(P_2[Year > 2017], function(x) median(x, na.rm = TRUE)) - sapply(P_2[Year <= 2017], function(x) median(x, na.rm = TRUE))
P_avg_slp2 <- as.data.frame(P_avg_diff[-1]*10/P_avg_diffyr) #This is the average slope. Units are microgram/Liter/year
colnames(P_avg_slp2) <- c('Avg Slope (μg/L/decade)')

# Slope distribution histogram
ggplot(P_avg_slp2, aes(x = `Avg Slope (μg/L/decade)`)) +
  geom_histogram(binwidth = 0.9) +
  geom_vline(xintercept = 0, linetype = 'twodash', color = 'grey', size = 1) +
...
median_slopes[2,] <- c(median(N_avg_slp2[,1]), median(P_avg_slp2[,1]))
slopeSD[2,] <- c(sd(N_avg_slp2[,1]),sd(P_avg_slp2[,1]))
mean_slopes[2,] <- c(mean(N_avg_slp2[,1]), mean(P_avg_slp2[,1]))
...
# Calculates the baseline and recent averages, then calculates the difference
N_avg_diff <- sapply(N_3[Year > 2013], function(x) median(x, na.rm = TRUE)) - sapply(N_3[Year <= 2009 & Year >= 1979], function(x) median(x, na.rm = TRUE))
N_avg_slp3 <- as.data.frame(N_avg_diff[-1]*10/N_avg_diffyr) #This is the average slope. Units are μmicrogram/Liter/year (μg/L/decade)
colnames(N_avg_slp3) <- c('Avg Slope (μg/L/decade)')

# Slope distribution histogram
ggplot(N_avg_slp3, aes(x = `Avg Slope (μg/L/decade)`)) +
...
P_avg_diff <- sapply(P_3[Year > 2013], function(x) median(x, na.rm = TRUE)) - sapply(P_3[Year <= 2009 & Year >= 1979], function(x) median(x, na.rm = TRUE))
P_avg_slp3 <- as.data.frame(P_avg_diff[-1]*10/P_avg_diffyr) #This is the average slope. Units are microgram/Liter/year
colnames(P_avg_slp3) <- c('Avg Slope (μg/L/decade)')

# Slope distribution histogram
ggplot(P_avg_slp3, aes(x = `Avg Slope (μg/L/decade)`)) +
  ...
  ggtitle('P Slope Distribution Histogram Treatment 3, bin-width = 0.8') 
#  scale_x_continuous(breaks = )



median_slopes[3,] <- c(median(N_avg_slp3[,1]), median(P_avg_slp3[,1]))
slopeSD[3,] <- c(sd(N_avg_slp3[,1]),sd(P_avg_slp3[,1]))
mean_slopes[3,] <- c(mean(N_avg_slp3[,1]), mean(P_avg_slp3[,1]))
slopeIQR <- list(N1 = quantile(N_avg_slp1[,1], type = 8), N2 = quantile(N_avg_slp2[,1], type = 8), N3 = quantile(N_avg_slp3[,1], type = 8),
                 P1 = quantile(P_avg_slp1[,1], type = 8), P2 = quantile(P_avg_slp2[,1], type = 8), P3 = quantile(P_avg_slp3[,1], type = 8))
