...
DOFrame <- marsFrame %>%  mutate(month = as.Date(Date, "%B")) %>%
  group_by(MonthSeq.y) %>%
    summarize(logDO = as.double(mean(ConcAve, na.rm = TRUE)))
...
plotConcTimeDaily(eList)
autoplot.marssMLE(marssfit2,silent = TRUE)
...
DOFrame$Est <- DOFrame$States[,"X1"]*Z[1,1,] + DOFrame$States[,"X2"]*yearFrame$decyear + DOFrame$States[,"X3"]*QFrame$LogQ + DOFrame$States[,"X4"]*sineFrame$SinDY + DOFrame$States[,"X5"]*cosFrame$CosDY
