...
These plot show a large gap from ~2008 to ~2012
There are a lot of 0's in the data set because two columns are merged, these need to be removed
...
From the WRTDS_Sample_Workflow file, it looks like the sample dataframe needs a number of fields that were not imported from the KC database, so they have to be added manually
...
GreenDO %>%
  ...
  ggtitle("Green River DO")
```
Much better, This almost looks like the DO is steadily decreasing or staying constant
