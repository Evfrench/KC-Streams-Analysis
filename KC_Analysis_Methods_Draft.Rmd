...
modlist1 = list( #all other arguments are left at default
  ...
  A = "zero",
  B = "identity",
  Q = ldiag(list(0.002,"q2","q3","q4","q5"))
)
...
Z2=Z
dim(Z2)= c(5,dim(Z)[3])
A = Z2[,1:3] %*% marssfit$states[1:3,]
plot(diag(A))

TT=dim(Z)[3]
lev = rep(NA, TT)
for (i in 1:TT){
  lev[i] = Z[1, 1:3,i] %*% marssfit$states[1:3,i]
}
plot(lev)
apply(Z,2,function(x){any(is.na(x))})

plot(Daily$waterYear)
poly() #use this for making yearly effects 
#otherwise they will be highly correlated 
```
MARSS DLM Residuals: 
