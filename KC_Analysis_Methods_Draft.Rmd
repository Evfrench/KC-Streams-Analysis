...
testdf <- data_frame()
testdf <- marsFrame %>% mutate(month = as.Date(Date, "%B")) %>%
    group_by(MonthSeq.y) %>%
    summarize(decimalYear = mean(DecYear.y)) %>%
  mutate(month = as.Date(Date, "%B")) %>%
    group_by(MonthSeq.y) %>%
    summarize(SinDY = mean(SinDY)) %>%
  mutate(month = as.Date(Date, "%B")) %>%
    group_by(MonthSeq.y) %>%
    summarize(CosDY = mean(CosDY)) %>% 
  mutate(month = as.Date(Date, "%B")) %>%
  group_by(MonthSeq.y) %>%
    summarize(logDO = log(mean(ConcAve))) 
