...
errorStats(eList2) 
```
R-Squared Value is 0.831, the flux is 0.925
...
rSquared(errtest$ConcAve, errtest$reds)

```
R-squared for DLM T model is 0.863, R-square for EGRET T is 0.912
...
ggplot() +
  ...
  geom_line(data = Sample2, aes(x = Date, y = DOest), color = "red") +
  geom_line(data = eList$Daily, aes(x = Date, y = ConcDay), color = "darkgreen") +
   geom_line(data = DOFrame, aes(x = Date, y = Est), color = "darkgreen") +
  ggtitle("Green River DO, Observed vs Henry's law Estimate & 2 Models")
